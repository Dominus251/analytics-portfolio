...
- обеспечить интеграцию с Power BI по упрощённой схеме настройки источников.
...
После завершения работы автоматически формируются пять наборов данных.
...
- количество сессий среди платящих и не платящих пользователей
...
| **Forecast** | Операционный прогноз на 1–12 месяцев. Показывает общую картину по продукту (игроки, выручка, прибыль), сделан отдельно для возможности масштабирования не меняя структуры `Scenario`. |
...
| **CAC Mode** | **Historical** - расчет опираясь на исторические параметры / **Manual** - расчет из параметров пользователя |
| **Manual Mix Mode** | Historical Mix* / Manual Mix* |
| **Organic** / **Ads** / **Referral Share** | Доли каналов - 100% максимум регулируются [автоматически](../scripts/apps_scripts.gs)|
...
- активные пользователи ─ это пользователи с покупокой или активной подпиской.
